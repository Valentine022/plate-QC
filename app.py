...
# Heavy libraries load only after authentication succeeds.
import base64
import io
import html
...
use_default_threshold = st.checkbox(
    ...
    value=True,
    key="default_threshold_checkbox",
)
...
if uploaded_file is None:
    st.info("Choose a CSV file, then click Generate report.")
    st.stop()
    ...
    try:
        with st.spinner("Generating report..."):
            with tempfile.TemporaryDirectory(prefix="plate_qc_") as temp_dir:
                ...
                st.session_state["report_stats"] = stats_path.read_bytes()
                st.session_state["report_hits"] = hits_path.read_bytes()
                st.session_state["report_name"] = safe_stem
...
if "report_html" in st.session_state:
    base_name = st.session_state["report_name"]
    ...
    st.download_button(
        ...
        data=st.session_state["report_stats"],
